...
public class ItemTatamiSlab extends ItemBlock {
	public ItemTatamiSlab(Block block) {
		super(block);
	}
	@Override
	public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int blockX, int blockY, int blockZ, Side side, double xPlaced, double yPlaced) {
		...
		Side slabSide = Side.getSideById(meta & 0b111);
		boolean doubled = (meta & 0b1000) != 0;
		if (Block.blocksList[world.getBlockId(blockX, blockY, blockZ)] != null && Block.blocksList[world.getBlockId(blockX, blockY, blockZ)].hasTag(BlockTags.PLACE_OVERWRITES)) {
			id = 0;
			meta = 0;
			slabSide = Side.getSideById(meta & 0b111);
			doubled = (meta & 0b1000) != 0;
		}
		...
		if (id == this.blockID && (side == slabSide.getOpposite() && !doubled)) {
			...
			world.setBlockMetadataWithNotify(blockX, blockY, blockZ, 0b1000 | meta);
		...
		if (id == this.blockID && (yPlaced > 0.5 && !doubled || yPlaced <= 0.5 && !doubled || (side == slabSide && !doubled))) {
			AABB bbBox = AABB.getBoundingBoxFromPool(blockX, blockY, blockZ, (float)blockX + 1.0f, (float)blockY + 1.0f, (float)blockZ + 1.0f);
			if (!world.checkIfAABBIsClear(bbBox)) {
				return false;
			}
